...
export async function getPurchaseOrders() {
  const [{ data: pos, error: e1 }, { data: shipments, error: e2 }] = await Promise.all([
...
export async function getPoLines(po_id) {
  // Strip shipment suffix e.g. TSH123UKSEA -> TSH123
  const base_id = po_id.replace(/(USA|UK|US)(SEA|AIR|TRUCK)?$/i, '').trim()
...
export async function updatePoLine(id, updates) {
